...
use crate::{
    id::encode_matout_id,
    model::{MatOutEntry, MaterializedOutputStore, MaterializedReferrer, Node, NodeMaterialized, PortKind, ProducedBy},
};
...
fn track_upsert(mutation: &mut MaterializedMutation, store: &MaterializedOutputStore, id: &str) {
    if let Some(entry) = store.get(id) {
        mutation.upserted_entries.insert(id.to_string(), entry.clone());
    }
}
...
fn gc_if_unreferenced(store: &mut MaterializedOutputStore, id: &str, mutation: &mut MaterializedMutation) {
    let should_delete = store.get(id).map(|entry| entry.referrers.is_empty()).unwrap_or(false);
    if should_delete {
        store.remove(id);
...
pub fn add_referrer(store: &mut MaterializedOutputStore, id: &str, referrer: MaterializedReferrer, mutation: &mut MaterializedMutation) {
...
pub fn remove_referrer(store: &mut MaterializedOutputStore, id: &str, referrer: &MaterializedReferrer, mutation: &mut MaterializedMutation) {
    let Some(entry) = store.get_mut(id) else {
        return;
    };
    entry.referrers.retain(|candidate| candidate.node_id != referrer.node_id || candidate.key != referrer.key);
    gc_if_unreferenced(store, id, mutation);
}

pub fn set_node_input_ref(node: &mut Node, key: &str, id: Option<String>, store: &mut MaterializedOutputStore) -> MaterializedMutation {
    let mut mutation = MaterializedMutation::default();
    if let Some(current_id) = node.materialized.inputs.remove(key) {
        remove_referrer(
            store,
            &current_id,
            &MaterializedReferrer { node_id: node.id.clone(), key: key.to_string() },
            &mut mutation,
        );
    }
    if let Some(id) = id {
...
pub fn set_node_output_ref(node: &mut Node, port: &str, id: Option<String>, store: &mut MaterializedOutputStore) -> MaterializedMutation {
    let mut mutation = MaterializedMutation::default();
    if let Some(current_id) = node.materialized.outputs.remove(port) {
        remove_referrer(
            store,
            &current_id,
            &MaterializedReferrer { node_id: node.id.clone(), key: port.to_string() },
            &mut mutation,
        );
    }
    if let Some(id) = id {
        node.materialized.outputs.insert(port.to_string(), id.clone());
        add_referrer(
            store,
            &id,
            MaterializedReferrer { node_id: node.id.clone(), key: port.to_string() },
...
#[allow(dead_code)]
pub fn clear_node_materialized(node: &mut Node, store: &mut MaterializedOutputStore) -> MaterializedMutation {
    let mut mutation = MaterializedMutation::default();
    ...
    for key in node.materialized.outputs.keys().cloned().collect::<Vec<_>>() {
...
#[allow(dead_code)]
pub fn duplicate_node_materialized(source: &Node, target: &mut Node, store: &mut MaterializedOutputStore) -> MaterializedMutation {
    let mut mutation = MaterializedMutation::default();
...
pub fn decode_entry_bytes(entry: &MatOutEntry) -> Vec<u8> {
    base64::engine::general_purpose::STANDARD.decode(&entry.data_base64).unwrap_or_default()
}

#[allow(dead_code)]
pub fn resolve_input_bytes(node: &Node, key: &str, store: &MaterializedOutputStore) -> Option<Vec<u8>> {
    let id = node.materialized.inputs.get(key)?;
    store.get(id).map(decode_entry_bytes)
}

#[allow(dead_code)]
pub fn resolve_output_bytes(node: &Node, port: &str, store: &MaterializedOutputStore) -> Option<Vec<u8>> {
